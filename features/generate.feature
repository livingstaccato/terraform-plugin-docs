...
Feature: Generate documentation for a Terraform provider
  ...
  Scenario: Ignoring deprecated resources and data sources
    Given a Terraform provider with a deprecated resource "deprecated_resource"
    And a `schema.json` file for the provider with the deprecated resource
    When I run `tfplugindocs generate --providers-schema=schema.json --ignore-deprecated=true`
    Then the file "docs/resources/deprecated_resource.md" should not be generated

  Scenario: Using a rendered provider name
    Given a Terraform provider with the provider name "test-provider"
    And a `schema.json` file for the provider
    When I run `tfplugindocs generate --providers-schema=schema.json --rendered-provider-name="My Awesome Provider"`
    Then the file "docs/index.md" should contain "# My Awesome Provider"

  Scenario: Generating documentation for functions
    Given a Terraform provider with a function "test_function"
    And a `schema.json` file for the provider with the function
    When I run `tfplugindocs generate --providers-schema=schema.json`
    Then the file "docs/functions/test_function.md" should be generated

  Scenario: Generating documentation for ephemeral resources
    Given a Terraform provider with an ephemeral resource "test_ephemeral_resource"
    And a `schema.json` file for the provider with the ephemeral resource
    When I run `tfplugindocs generate --providers-schema=schema.json`
    Then the file "docs/ephemeral-resources/test_ephemeral_resource.md" should be generated
